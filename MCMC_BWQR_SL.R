...
bayesQRSL_weighted <- function(y,x,w,tau,n_mcmc,burnin_mcmc,thin_mcmc){
  ...
  # MCMC
  for(k in 2:n_mcmc){
    beta_aux   <- atualizarBETA_MH(rep(0,numcov),diag(rep(1000,numcov)),y,x,w,beta[k-1,],tau,const[k-1],k)
...
atualizarBETA_MH <- function(b,B,y,x,w,beta,tau,ct,k){
  valoratual    <- beta
  sigma_MH      <- (tau*(1-tau))*chol2inv(chol((1/length(y))*t(w^2*x)%*%x))
  valorproposto <- as.vector(rmvnorm(1, mean=valoratual, sigma=ct*sigma_MH))
  candidato     <- exp(condicionalBETA_MH(valorproposto,b,B,y,x,w,tau)-
                       condicionalBETA_MH(valoratual,b,B,y,x,w,tau))
...
condicionalBETA_MH <- function(beta,b,B,y,x,w,tau){
  n       <- length(y)
